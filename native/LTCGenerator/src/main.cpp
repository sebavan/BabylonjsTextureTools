...
void computeAvgTerms(const Brdf& brdf, const vec3& V, const float alpha,
    float& norm, float& fresnel, vec3& averageDir, int Nsample)
{
    norm = 0.0f;
    fresnel = 0.0f;
...
float computeError(const LTC& ltc, const Brdf& brdf, const vec3& V, const float alpha, int Nsample)
{
    double error = 0.0;
...
struct FitLTC
{
    FitLTC(LTC& ltc_, const Brdf& brdf, bool isotropic_, const vec3& V_, float alpha_, int Nsample_) :
        ltc(ltc_), brdf(brdf), V(V_), alpha(alpha_), isotropic(isotropic_), Nsample(Nsample_)
    {
    }
    ...
    {
        update(params);
        return computeError(ltc, brdf, V, alpha, Nsample);
    }
...
void fit(LTC& ltc, const Brdf& brdf, const vec3& V, const float alpha, const float epsilon = 0.05f, const bool isotropic = false, int Nsample = 32)
{
    float startFit[3] = { ltc.m11, ltc.m22, ltc.m13 };
    float resultFit[3];

    FitLTC fitter(ltc, brdf, isotropic, V, alpha, Nsample);
...
// fit data
void fitTab(mat3* tab, vec2* tabMagFresnel, const int N, const Brdf& brdf, int NSamples, float MIN_ALPHA)
{
    LTC ltc;
    ...
    for (int t =     0; t <= N - 1; ++t)
    {
        ...
        vec3 averageDir;
        computeAvgTerms(brdf, V, alpha, ltc.magnitude, ltc.fresnel, averageDir, NSamples);
        ...
        float epsilon = 0.05f;
        fit(ltc, brdf, V, alpha, epsilon, isotropic, NSamples);
...
emscripten::val BuildLTC(int N, int Nsample, float MIN_ALPHA) {
    ...
    // fit
    fitTab(tab, tabMagFresnel, N, brdf, Nsample, MIN_ALPHA);

    // projected solid angle of a spherical cap, clipped to the horizon
    genSphereTab(tabSphere, N);

    size_t elementCount = N * N;
    vec4* ltcMem = new vec4[elementCount * 2];
    vec4* ltc2 = &ltcMem[0];
    vec4* ltc1 = &ltcMem[elementCount];
...
// EMSCRIPTEN_BINDINGS macro to expose the function to JavaScript
